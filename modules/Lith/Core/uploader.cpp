...
void Uploader::upload(const QString& path) {
    ...
    if (!file->open(QIODevice::ReadOnly)) {
        emit error(tr("Could not open file %1").arg(path));
        return;
    }

    workingSet(true);
    QNetworkRequest request(url);
    request.setHeader(QNetworkRequest::ContentTypeHeader, QStringLiteral("application/application/x-www-form-urlencoded"));
