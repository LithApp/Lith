...
void QmlObjectList::append(const QVariantMap& properties) {
    ...
    if (newObj == Q_NULLPTR) {
        Lith::instance()->log(
            Logger::Unexpected, QStringLiteral(QT_TR_NOOP("Object model cannot instantiate %1, missing constructor"))
                                    .arg(QString::fromUtf8(mMetaObject->className()))
        );
        return;
    }
    for (auto [key, value] : properties.asKeyValueRange()) {
        if (!newObj->setProperty(key.toUtf8().data(), value)) {
            Lith::instance()->log(
                Logger::Unexpected, QStringLiteral(QT_TR_NOOP("Object model cannot append object of type %1 with property %2"))
                                        .arg(QString::fromUtf8(mMetaObject->className()))
                                        .arg(key)
            );
        }
